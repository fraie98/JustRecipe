...
\newpage
\subsection{Neo4J}
Now we will talk about the queries on Neo4j, but first we need to consider their domain-specific to graph-centric translation in the table \ref{table: Graph-Centric Query and Domain-Specific Query}
\begin{table}[htpb]
\caption{Graph-Centric Query and Domain-Specific Query}
\centering
\label {table: Graph-Centric Query and Domain-Specific Query}
\begin{tabular}{|l|l|}
\hline
\rowcolor[HTML]{000000} 
...
\begin{table}[htpb]
	...
	\label{table: Test on creationTime Index} 
	\centering
...
\lstset{language=code}
\begin{lstlisting}
	db.recipes.createIndex({creationTime:-1},{name:'creationTime_index'})
\end{lstlisting}


\subsubsection{Index on Comment Creation Time}
A possible index is the one on the creation time of the comment, this index affects the query \emph{Last Comments}. In order to see if the index must be added or not, let analyze the result of the index analysis for this index on that query (see table \ref{table: Statistics about comment creation time index})


\begin{table}[htpb]
	\centering
	\caption{Statistics about comment creation time index}
	\label{table: Statistics about comment creation time index}
	\begin{tabular}{lllll}
		\cline{1-3}
		...
		\multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}ExecutionTimeMillis: 123ms\\ TotalKeysExamined: 0\\ TotalDocsExamined: 45348\end{tabular}} &
		\multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}ExecutionTimeMillis: 2ms\\ TotalKeysExamined: 18\\ TotalDocsExamined: 3\end{tabular}} &
		&
		\\ \cline{1-3}
	\end{tabular}
\end{table}

\noindent From the result obtained we can see that the addition of that index imply an improvement in the performance. In fact the query with the index requires less time with respect to the one without index.

\noindent For this reason we create an index on the creation time of the comment.

\noindent The reason for the improvement of the performance is because we create a sparse index, so the recipes in which there aren't comments are not taken into account. Thus if the index is present it examines a number of keys that is lesser than the number of recipes, because not all recipes contain comments. In the test scenario are present 18 comments in 3 different recipes and in fact only these are taken into account with a huge reduction of examined docs.

\lstset{language=code}
\begin{lstlisting}
	db.recipes.createIndex({"comments.creationTime":-1},
	{sparse:true, name:'commentsCreationTime_index'})
\end{lstlisting}

\subsubsection{Index on Ingredients}
A possible \emph{text index} is the one on the ingredients due to the fact that we have to search them.

\noindent In order to test the text index we use the following query as example.
\lstset{language=code}
\begin{lstlisting}
db.recipes.explain("executionStats").aggregate([
		 {$match: {"$text": {"$search":"\"vanilla\" \"milk\" \"strawberries\""}}},
		 {$sort : {creationTime : -1 }},
		 { $skip: 0}, {$limit: 20}
	 ])
\end{lstlisting}

\noindent The results are the ones in the table \ref{table: Statistics about text index}.
...
\begin{table}[htpb]
	\centering
	\caption{Statistics about comment creation time index}
	\label{table: Statistics about text index}
	\begin{tabular}{cllll}
		\cline{1-3}
		\multicolumn{1}{|c|}{\cellcolor[HTML]{000000}{\color[HTML]{FFFFFF} Query}} &
		\multicolumn{1}{c|}{\cellcolor[HTML]{000000}{\color[HTML]{FFFFFF} Results without index}} &
		...
		&
		\\ \cline{1-3}
		\multicolumn{1}{|c|}{\begin{tabular}[c]{@{}c@{}}Search For \\ Recipe's Ingredients\end{tabular}} &
		\multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}ExecutionTimeMillis: 91ms\\ TotalKeysExamined: 6514\\ TotalDocsExamined: 6514\end{tabular}} &
		\multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}ExecutionTimeMillis: 249ms\\ TotalKeysExamined: 12239\\ TotalDocsExamined: 998\end{tabular}} &
		&
		\\ \cline{1-3}
	\end{tabular}
\end{table}

\noindent Considering the results we can notice that without index the system uses the index on the creation time and for our queries that index is the one that ensures better performance.
...
\noindent Only the first 10 recipes are shown, so if we scroll until the end we can click on the button \textbf{Next} to see the next 10, and then the button \textbf{Previous} to see the previous 10. We can see this in figure 4.16 and 4.17.
